...
#         self.traffic_allocation[0] = 1 / (len(self.active_treatments) + 1)
#         for a in self.active_treatments:
#             self.traffic_allocation[a] = 1 / (len(self.active_treatments) + 1)
#         return
...
if 0:
    ...
    for batch in range(num_of_batches):
        ...
        std_for_rand = np.tile(std[:, :, np.newaxis], (1, 1, num_of_iter)) / np.sqrt(np.tile(new_arm_pulls[:, np.newaxis, :], (1, M, 1)))
