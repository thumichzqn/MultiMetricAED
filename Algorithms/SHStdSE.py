...
class SHStdSE_Vec:
    ...
    def update_traffic_allocation(self):
        ...
        :return:
        """
        std_est_max_traffic = np.max(self.std_est, axis = 1)
        std_est_max_traffic[1:, :] = std_est_max_traffic[1:, :] * self.active_treatments
        std_total_traffic = np.sum(std_est_max_traffic, axis = 0)
        self.traffic_allocation = std_est_max_traffic / np.tile(std_total_traffic[np.newaxis, :], (self.action_size + 1, 1))
        return
