import numpy as np
# Multi-Objective Optimization with Single Estimator
class MOOSE:
    ...
    def update_traffic_allocation(self):
        ...
        for arm in self.active_treatments:
            a = arm - 1
            rho_effective_treatment[a] = self.rho_est[a, :].max()
            rho_effective_control = max([rho_effective_control, self.lambda_est[a, :].max()])
        ...
        self.traffic_allocation[1:] = rho_square_effective_treatment / (
                    rho_treatment_total * (rho_treatment_total + rho_effective_control))
        return
...
class MOOSE_Vec:
    ...
    def update_std_est(self, std):
        """
        update the standard deviation and variance of arms
        :param std: standard deviation, numpy array (A+1, M, num_of_iter)
        :return:
        """
        self.std_est = std
        self.var_est = np.power(self.std_est, 2)
    ...
    def update_traffic_allocation(self):
        ...
        :return:
        """
        rho_effective_treatment = np.max(self.rho_est, axis = 1)
        ...
        self.traffic_allocation[1:, :] = rho_square_effective_treatment_active / np.tile(numerator[np.newaxis, :], (self.action_size, 1))
        return
